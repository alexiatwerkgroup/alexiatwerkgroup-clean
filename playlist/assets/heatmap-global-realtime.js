(function(){
  ...
  const MIN_TOTAL_REPLAYS_TO_SHOW = 3;
  const MIN_PEAK_REPLAYS_TO_SHOW = 2;
  ...
  function render(seconds){
    ...
    for (let sec = 0; sec <= safeMax; sec++) {
      const count = exactCounts[sec] || 0;
      if (count < MIN_PEAK_REPLAYS_TO_SHOW) continue;
      const idx = Math.max(0, Math.min(BUCKETS - 1, Math.floor((sec / safeMax) * BUCKETS)));
      buckets[idx].count += count;
      buckets[idx].sum += sec * count;
    }

    const counts = buckets.map(b => b.count);
    const totalReplays = counts.reduce((sum, n) => sum + n, 0);
    const maxCount = Math.max(1, ...counts);
    const step = s.w / Math.max(1, BUCKETS - 1);
    const baseY = s.h - 10;

    if (totalReplays < MIN_TOTAL_REPLAYS_TO_SHOW) {
      peakSecond = null;
      if (jumpBtn) jumpBtn.style.display = 'none';
      drawEmpty('No replay data yet');
      return;
    }
    ...
    if (bestSecond === null || bestCount < MIN_PEAK_REPLAYS_TO_SHOW) {
      peakSecond = null;
      if (jumpBtn) jumpBtn.style.display = 'none';
      drawEmpty('No replay data yet');
      return;
  ...
  function handlePlaybackTick(){
    ...
    // Normal playback does NOT count as replay heatmap data.
    lastSecond = sec;
  }
  ...
  function bindNative(){
    ...
    nativeVideo.addEventListener('seeking', function(){
      const nextSecond = Math.floor(nativeVideo.currentTime || 0);
      const prevSecond = Math.floor(lastSecond == null ? nextSecond : lastSecond);
      if (prevSecond - nextSecond >= Math.abs(REWIND_THRESHOLD) && nextSecond > START_GUARD_SECONDS) {
        noteSecond(nextSecond, 5);
      }
